...
async function mintMockWrappedTheta(chainIDStr, chainRPC, wrappedThetaContractAddr, receivingAddr, amount, minterKey, minterKeyPassword) {
    const provider = createProvider(chainIDStr, chainRPC);
    const minterWallet = createWallet(provider, minterKey, minterKeyPassword);
...
async function mintMockWrappedThetaOnMainchain(receiverAddr, amount, minterKey, minterKeyPassword) {
    let wrappedThetaContractAddr = cfg().wTHETAAddr;
    console.log('wrappedThetaContractAddr:',wrappedThetaContractAddr);
    let mintTx = await mintMockWrappedTheta(cfg().mainchainIDStr, cfg().mainchainRPC, wrappedThetaContractAddr, receiverAddr, amount, minterKey, minterKeyPassword);
...
module.exports = {
    mintMockWrappedThetaOnMainchain
};