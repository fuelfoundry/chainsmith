...
function getWallet(chainIDStr, rpc, key, keyPassword) {
    const provider = createProvider(chainIDStr, rpc);
    const wallet = createWallet(provider, key, keyPassword);
    return wallet;
}

// Create wallet
function createWallet(provider, key, password) {
    console.log('-------')
    console.log('--- ---')
    console.log('-------')

        const json = JSON.stringify(key); //fs.readFileSync(path.resolve(key));
        //console.log(json);
...
async function printValidatorSetForDynasty(substake_id, subchain_chainId, chainRegistrarOnMainchainContract, dynasty) {
    const {validators, shareAmounts} = await chainRegistrarOnMainchainContract.getValidatorSet(subchain_chainId, dynasty);
    ...
    console.log(JSON.stringify(validator_set, null, 2)); // 2 spaces for indentation
...
function getTokenBank(tokenType, chainID, senderKeyPath, senderKeyPassword) {
    ...
    const senderWallet = createWallet(provider, senderKey, senderKeyPassword);
